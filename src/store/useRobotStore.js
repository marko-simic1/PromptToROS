...
const ENV = {
  ...
  POSE_TOPIC: import.meta.env.VITE_POSE_TOPIC || '/ranger_1/pose',
...
const useRobotStore = create((set, get) => ({
  ...
  _setupTopics: (ros) => {
    ...
    const poseTopic = new ROSLIB.Topic({
      ros,
      name: ENV.POSE_TOPIC,
      messageType: ENV.POSE_TOPIC.includes('odom') ? 'nav_msgs/Odometry' : 'geometry_msgs/PoseStamped',
      throttle_rate: 100,
    });
    poseTopic.subscribe((msg) => {
      const isOdom = msg.pose?.pose !== undefined;
      const p = isOdom ? msg.pose.pose.position : msg.pose.position;
      const q = isOdom ? msg.pose.pose.orientation : msg.pose.orientation;
      const t = isOdom ? msg.twist?.twist : null;
      set({
        position: { x: p.x, y: p.y },
        orientation: { yaw: quatToYaw(q) },
        ...(t && { velocity: { linear: t.linear.x, angular: t.angular.z } }),
      });
    });
